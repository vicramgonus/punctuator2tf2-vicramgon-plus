...
def tokenize(sent, puncts=[',', '.', ';', ':', '?', '!']):
  ...
  for i in range(len(sent)):
    ...
    if c_i == ' '  or c_i in puncts:
      ...
      # Si la palabra empieza por mayúscula entonces añadimos delate el token de 
      # mayúscula
      if cur_read_type == 'string' and partial[0].isupper():
          res.append("·M")
...
def process_line(line, puncts= {".": ".PERIOD", ",": ",COMMA", ";": ";SEMICOLON", 
    ...
    for token in tokens:
        ...
        # Si el token corresponde a una marca de mayúscula
        elif token == '·M':
          # Entonces si el anterior token era un término, entonces se añade la marca
          if prev_token not in puncts:
            output_tokens.append(token)
          else:
            # En otro caso, se enlaza con el token de puntuación
            output_tokens[-1] += 'M'
